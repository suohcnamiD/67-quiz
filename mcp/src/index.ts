...
server.tool(
  'login',
  'Log in to 67-quiz with username and password. Must be called before any other tool.',
  {
...
server.tool(
  ...
  async ({ page }) => {
    try {
      ...
      if (quizzes.length === 0) return { content: [{ type: 'text', text: 'No quizzes found.' }] }
      const lines = quizzes.map((q: any) => `- ${q.name} (id: ${q.id}, questions: ${q.questionCount ?? 0})`)
...
server.tool(
  'create_quiz',
  'Create a new quiz. Returns the quiz ID.',
  {
...
server.tool(
  'delete_quiz',
  'Delete a quiz you authored.',
  { quiz_id: z.string().uuid().describe('The quiz ID to delete') },
  async ({ quiz_id }) => {
    try {
...
server.tool(
  'get_quiz',
  'Get full details of a quiz you authored, including all questions and options.',
  { quiz_id: z.string().uuid().describe('The quiz ID') },
  async ({ quiz_id }) => {
    try {
      const res = await http.get(`/quiz/authoring/${quiz_id}`)
      const quiz = res.data
      const lines: string[] = [`Quiz: ${quiz.name} (${quiz.duration})`, '']
